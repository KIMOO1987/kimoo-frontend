...
export default function Sidebar({ isPro }: { isPro: boolean }) {
  ...
  if (!mounted) {
    return (
      <div className="w-64 h-screen bg-[#05070a] border-r border-white/5 flex flex-col p-6" />
    );
  }

  return (
    <div className="w-64 h-screen bg-[#05070a] border-r border-white/5 flex flex-col p-6 sticky top-0 overflow-y-auto">
      {/* Brand Logo */}
      ...
      <nav className="flex-1 space-y-8">
        {menuGroups.map((group) => (
          <div key={group.label}>
            <p className="text-[9px] font-black text-zinc-700 tracking-[0.2em] mb-4 px-2">
              {group.label}
            </p>
            <div className="space-y-1">
              {group.items.map((item) => {
                ...
                return (
                  <Link key={item.name} href={isLocked ? "#" : item.path}>
                    <motion.div
      ...
      <div className="mt-auto pt-6 border-t border-white/5 space-y-4">
        <div className="flex items-center justify-between px-2">
          ...
          {userEmail && (
            <span className="text-[8px] font-mono text-zinc-600 truncate max-w-[100px]">
              {userEmail.split('@')[0]}
            </span>
          )}
        </div>
