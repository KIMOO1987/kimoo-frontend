...
import { approveUserAction, kickHashAction } from './actions';
import {
  Check,
  ExternalLink,
  Loader2,
...
export default function AdminPayments() {
  ...
  // FIXED: approveUser now calls a Server Action that verifies admin role server-side
  async function approveUser(userId: string, requestedPlan: string) {
    ...
    try {
      await approveUserAction(userId, requestedPlan);
      fetchData();
    } catch (err: any) {
      ...
      <div className="max-w-[1700px] mx-auto relative z-10">
        <div className="mb-8 md:mb-12 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-6xl">
          <div className="relative overflow-hidden bg-gradient-to-br from-blue-500/10 to-blue-900/10 border border-blue-500/20 p-6 md:p-8 rounded-[2.5rem] flex items-center justify-between shadow-[0_0_30px_rgba(59,130,246,0.15)]">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-black italic text-white tracking-tighter">${totalEarnings} <span className="text-sm not-italic text-zinc-500 ml-2 font-bold tracking-widest">USDT</span></h2>
            </div>
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/[0.05] p-6 md:p-8 rounded-[2.5rem] flex items-center justify-between shadow-2xl">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-black italic text-white tracking-tighter">{requests.length} <span className="text-sm not-italic text-zinc-500 ml-2 font-bold tracking-widest">Pending</span></h2>
            </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 md:mb-12 max-w-6xl">
          <div>
            ...
            <p className="text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-bold mt-3 leading-none">• CRYPTO PAYMENT VALIDATION QUEUE •</p>
          </div>
                  ...
                  <div key={req.id} className="relative overflow-hidden bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/[0.05] p-6 md:p-8 rounded-[2.5rem] flex flex-col lg:flex-row items-start lg:items-center justify-between group hover:border-white/[0.1] transition-all duration-300 shadow-2xl gap-6">
                    <div className="space-y-4 flex-1 relative z-10 w-full">
                      <div className="flex flex-wrap items-center gap-4">
                        <span className="text-base font-black italic uppercase tracking-tighter text-white">{req.email}</span>
                        <span className="flex items-center gap-1.5 text-[9px] font-bold text-zinc-400 bg-white/[0.03] px-3 py-1 rounded-lg border border-white/5">
                      ...
                      <div className="flex items-center gap-3 bg-white/[0.02] p-3 rounded-xl border border-white/[0.05] w-full lg:w-fit">
                        ...
                        <a href={`https://tronscan.org/#/transaction/${req.pending_crypto_hash}`} target="_blank" className="p-2 bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.1] rounded-lg text-zinc-400 hover:text-white transition-colors ml-auto">
                    ...
                    <div className="relative z-10 flex items-center gap-4 w-full lg:w-auto mt-2 lg:mt-0">
                      ...
                      <button onClick={() => approveUser(req.id, req.pending_plan_id)} className="flex-1 lg:flex-none flex items-center justify-center gap-3 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white px-10 py-4 rounded-xl font-black text-[11px] uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(16,185,129,0.3)] active:scale-95 border border-emerald-500/30 transition-all">
                        <Check size={18} />
                        Approve
                      </button>
                    </div>
