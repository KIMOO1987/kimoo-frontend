          ...
          <div className="fixed inset-0 /80 backdrop-blur-2xl z-[100] flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
            <div className="w-full max-w-xl p-8 md:p-10 bg-gradient-to-br from-[#0a0c10] to-[#030407] border border-[var(--glass-border)] rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.8)] max-h-[90vh] overflow-y-auto relative">
              ...
              <div className="relative z-10">
                ...
                <div className="bg-gradient-to-br from-white/[0.04] to-white/[0.01] rounded-2xl p-5 mb-8 border border-[var(--glass-border)] flex justify-between items-center shadow-lg">
                    <span className="text-sm font-black uppercase tracking-widest text-zinc-900 dark:text-white">{selectedPlan.name}</span>
                ...
                <div className="flex gap-2 mb-8 bg-[var(--glass-bg)] p-1.5 rounded-2xl border border-[var(--glass-border)]">
                  <button 
                    onClick={() => setPaymentMethod('WHOP')}
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${paymentMethod === 'WHOP' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-zinc-900 dark:text-white shadow-[0_0_20px_rgba(59,130,246,0.3)]' : 'text-zinc-600 dark:text-zinc-500 hover:text-zinc-900 dark:text-white hover:bg-[var(--glass-bg)]'}`}
                  >
                    <CreditCard size={14} /> Card
                  </button>
                  <button 
                    onClick={() => setPaymentMethod('CRYPTO')}
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${paymentMethod === 'CRYPTO' ? 'bg-gradient-to-r from-orange-600 to-red-600 text-zinc-900 dark:text-white shadow-[0_0_20px_rgba(249,115,22,0.3)]' : 'text-zinc-600 dark:text-zinc-500 hover:text-zinc-900 dark:text-white hover:bg-[var(--glass-bg)]'}`}
                  >
                ...
                {paymentMethod === 'WHOP' ? (
                  ...
                  >
                    {loading ? <Loader2 className="animate-spin text-blue-200" size={18}/> : <><CreditCard size={18} className="text-blue-200" /> <span className="text-[11px] font-black uppercase tracking-[0.2em] text-zinc-900 dark:text-white">Pay with Whop Checkout</span></>}
                  </button>
                ) : (
