        ...
        <div className="max-w-[1700px] mx-auto relative z-10 flex flex-col min-h-screen space-y-8">
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6 mb-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full lg:w-auto">
              ...
              <div className="flex flex-col gap-1 w-full md:w-48">
                ...
                <select value={assetClass} onChange={(e) => setAssetClass(e.target.value)} className="bg-white/[0.02] border border-white/[0.08] rounded-xl px-4 py-2.5 text-xs font-mono font-bold text-white outline-none focus:border-blue-500/50 hover:border-white/20 transition-all cursor-pointer appearance-none w-full">
              ...
              <div className="relative flex-grow md:w-64 self-end h-[42px] mb-0.5">
                ...
                <input type="text" placeholder="Filter Symbol..." className="w-full h-full pl-12 pr-4 bg-white/[0.02] border border-white/[0.08] rounded-xl text-xs font-mono text-white focus:border-blue-500/50 hover:border-white/20 outline-none transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
            </div>
          </div>
                    ...
                    {history.length > 0 ? (
                      history.map((signal) => (
                        ...
                        >
                          ...
                          <td className="py-6 hidden md:table-cell text-xs font-mono text-zinc-500">{new Date(signal.created_at).toLocaleDateString()}</td>
                          <td className="px-6 md:px-8 py-6 text-center">
                            <button className="p-2 rounded-xl bg-white/[0.02] border border-white/[0.05] text-zinc-500 hover:bg-white/[0.08] hover:text-white transition-all">
          ...
          {totalPages > 1 && (
            <div className="mt-8 mb-4 flex justify-center items-center gap-4">
              ...
              <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.05] text-zinc-500 hover:bg-white/[0.05] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronRightIcon size={20} /></button>
            </div>
          )}
...
function StatCard({ label, value, icon, color }: any) {
  return (
    <div className="relative overflow-hidden bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/[0.05] p-6 md:p-8 rounded-[2rem] hover:border-white/[0.1] hover:bg-white/[0.06] transition-all duration-500 group shadow-2xl flex flex-col justify-between">
      ...
      <div className="relative z-10 flex justify-between items-start mb-4 md:mb-6">
        ...
        <div className={`p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05] ${color} group-hover:scale-110 transition-transform duration-300 shadow-lg`}>{icon}</div>
      </div>
