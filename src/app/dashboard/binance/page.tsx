...
const fetchBotData = async (isSilentRefresh = false) => {
  ...
  useEffect(() => {
    ...
    const logChannel = supabase.channel(`binance-logs-stream-${userId}`)
      .on('postgres_changes', 
        ...
        (payload) => {
          if (payload.new.message) {
            setLogs(prev => {
              ...
              localStorage.setItem('binance_terminal_logs', JSON.stringify(newLogs));
              return newLogs;
            });
          }
        }
      ...
      <div className="max-w-[1700px] mx-auto relative z-10 space-y-6 md:space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 md:mb-12">
          ...
          <div className="flex items-center gap-3 px-5 py-2.5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl backdrop-blur-md shadow-xl">
            {userId && <BotStatus userId={userId} exchangeName="binance" cachedIsBotEnabled={isBotEnabled} />}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-4 space-y-6 md:space-y-8 order-2 lg:order-1">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex bg-[var(--glass-bg)] rounded-xl p-1.5 border border-[var(--glass-border)]">
                    <button 
                      ...
                      className={`flex-1 py-2.5 text-[10px] font-black rounded-lg transition-all uppercase tracking-widest ${environment === 'testnet' ? 'bg-yellow-500 text-black shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 'text-zinc-600 dark:text-zinc-500 hover:text-zinc-900 dark:text-white'}`}
                    >
                      TESTNET
                    </button>
                    <button 
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-3.5 text-xs font-mono text-zinc-900 dark:text-white outline-none focus:border-yellow-500/50 hover:border-white/20 transition-all"
                ...
                {botConfig?.exchange_name === 'binance' && (
                  <div className="space-y-2">
            ...
            <div className="bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-[var(--glass-border)] p-6 md:p-8 rounded-[2.5rem] shadow-2xl backdrop-blur-md space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                      ...
                      <input 
                        ...
                        className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-3 text-xs font-mono text-zinc-900 dark:text-white outline-none focus:border-yellow-500/50 hover:border-white/20 transition-all"
                      />
                  </div>
                  <div className="space-y-2">
              ...
              <div className="py-2">
                  <label className="flex items-center gap-3 cursor-pointer group bg-[var(--glass-bg)] p-4 rounded-xl border border-[var(--glass-border)] hover:border-yellow-500/30 hover:bg-[var(--glass-bg)] transition-all">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className="text-[11px] uppercase font-black tracking-widest text-zinc-700 dark:text-zinc-400 group-hover:text-yellow-400 transition-colors">
                  ...
                  <div className="absolute z-50 top-[100%] left-0 w-full mt-2 bg-[var(--input-bg)] border border-[var(--glass-border)] rounded-xl shadow-2xl p-3 max-h-[250px] overflow-y-auto flex flex-col gap-1">
                    ...
                    {AVAILABLE_GRADES.map(grade => (
                      <label key={grade} className="flex items-center gap-3 cursor-pointer group hover:bg-[var(--glass-bg)] p-2 rounded-lg transition-all border border-transparent hover:border-[var(--glass-border)]">
                        ...
                        />
                      <span className="text-[10px] uppercase font-black tracking-widest text-zinc-800 dark:text-zinc-300 group-hover:text-yellow-400">{grade} Setup</span>
                      </label>
                    ))}
                  </div>
                )}
                  ...
                  <div className="absolute z-50 top-[100%] left-0 w-full mt-2 bg-[var(--input-bg)] border border-[var(--glass-border)] rounded-xl shadow-2xl p-3 max-h-[250px] overflow-y-auto flex flex-col gap-1">
                    ...
                    {POPULAR_SYMBOLS.map(sym => (
                      <label key={sym} className="flex items-center gap-3 cursor-pointer group hover:bg-[var(--glass-bg)] p-2 rounded-lg transition-all border border-transparent hover:border-[var(--glass-border)]">
                        ...
                        className="accent-yellow-500 w-4 h-4 cursor-pointer"
                        />
                      <span className="text-[10px] uppercase font-black tracking-widest text-zinc-800 dark:text-zinc-300 group-hover:text-yellow-400">{sym}</span>
                      </label>
                    ))}
                  </div>
                )}
              ...
              <button 
                ...
                className={`w-full py-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-3 active:scale-95 border border-[var(--glass-border)] ${
                  environment === 'live' 
                    ...
                    : 'bg-gradient-to-r from-yellow-600 to-orange-500 text-zinc-900 dark:text-white shadow-[0_0_30px_rgba(234,179,8,0.3)] hover:shadow-[0_0_40px_rgba(234,179,8,0.5)]'
                }`}
              >
