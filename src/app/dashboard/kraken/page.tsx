...
const fetchBotData = async (isSilentRefresh = false) => {
    ...
    const [profileRes, authRes] = await Promise.all([
      ...
      supabase.from('exchange_auth')
        .select('*')
        .eq('user_id', user.id)
        .eq('exchange_name', 'kraken') // 🚨 CHANGE THIS based on the page (okx, mexc, etc.)
        .single()
    ]);
    ...
    if (authRes.data) {
      ...
      localStorage.setItem('kraken_data_cache', JSON.stringify({ userId: user.id, tier: profileRes.data?.tier || 0, data }));
    } else {
      localStorage.setItem('kraken_data_cache', JSON.stringify({ userId: user.id, tier: profileRes.data?.tier || 0, data: null }));
    }
