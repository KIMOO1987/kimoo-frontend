...
export default function KrakenDashboard() {
  ...
  const fetchBotData = async (isSilentRefresh = false) => {
    if (!isSilentRefresh) {
      // Optimistic Cache Load (Kraken specific)
      const cached = localStorage.getItem('kraken_data_cache');
      if (cached) {
        try {
    ...
    // FETCH FROM UNIFIED TABLE FILTERED BY KRAKEN
    ...
    if (data) {
      ...
      localStorage.setItem('kraken_data_cache', JSON.stringify({ userId: user.id, tier: profile?.tier || 0, data }));
    } else {
      localStorage.setItem('kraken_data_cache', JSON.stringify({ userId: user.id, tier: profile?.tier || 0, data: null }));
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const fetchRecentLogs = async () => {
      ...
      if (data && data.length > 0) {
        ...
        setLogs((prev) => prev.length === 0 ? [`[${new Date().toLocaleTimeString()}] SYSTEM: Kraken Secure connection established. Awaiting new signals...`] : prev);
      }
    };
    fetchRecentLogs();
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 md:mb-12">
          <div>
            <h1 className="text-2xl md:text-4xl font-black tracking-tighter italic flex items-center gap-3 uppercase text-white">
              <img src="/kraken-logo.png" alt="Kraken" className="w-8 h-8 md:w-10 md:h-10 object-contain drop-shadow-[0_0_15px_rgba(147,51,234,0.5)]" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-4 space-y-6 md:space-y-8 order-2 lg:order-1">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex bg-white/[0.02] rounded-xl p-1.5 border border-white/[0.08]">
                    <button 
                      ...
                      className={`flex-1 py-2.5 text-[10px] font-black rounded-lg transition-all uppercase tracking-widest ${environment === 'testnet' ? 'bg-purple-500 text-white shadow-[0_0_15px_rgba(147,51,234,0.4)]' : 'text-zinc-500 hover:text-white'}`}
                    >
                      SANDBOX
                    </button>
                    <button 
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/[0.02] border border-white/[0.08] rounded-xl px-4 py-3.5 text-xs font-mono text-white outline-none focus:border-purple-500/50 hover:border-white/20 transition-all"
            ...
            <div className="bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/[0.05] p-6 md:p-8 rounded-[2.5rem] shadow-2xl backdrop-blur-md space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                      ...
                      <input 
                        ...
                        className="w-full bg-white/[0.02] border border-white/[0.08] rounded-xl px-4 py-3 text-xs font-mono text-white outline-none focus:border-purple-500/50 hover:border-white/20 transition-all"
                      />
                  </div>
                  <div className="space-y-2">
              ...
              <div className="py-2">
                  <label className="flex items-center gap-3 cursor-pointer group bg-white/[0.02] p-4 rounded-xl border border-white/[0.05] hover:border-purple-500/30 hover:bg-white/[0.04] transition-all">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className="text-[11px] uppercase font-black tracking-widest text-zinc-400 group-hover:text-purple-400 transition-colors">
                  ...
                  <div className="absolute z-50 top-[100%] left-0 w-full mt-2 bg-[#0a0a0a] border border-white/[0.08] rounded-xl shadow-2xl p-3 max-h-[250px] overflow-y-auto flex flex-col gap-1">
                    ...
                    {AVAILABLE_GRADES.map(grade => (
                      <label key={grade} className="flex items-center gap-3 cursor-pointer group hover:bg-white/[0.02] p-2 rounded-lg transition-all border border-transparent hover:border-white/[0.05]">
                        ...
                        />
                        <span className="text-[10px] uppercase font-black tracking-widest text-zinc-300 group-hover:text-purple-400">{grade} Setup</span>
                      </label>
                    ))}
                  </div>
                )}
                    ...
                    {POPULAR_SYMBOLS.map(sym => (
                      <label key={sym} className="flex items-center gap-3 cursor-pointer group hover:bg-white/[0.02] p-2 rounded-lg transition-all border border-transparent hover:border-white/[0.05]">
                        <input 
                          ...
                          className="accent-purple-500 w-4 h-4 cursor-pointer"
                        />
                        <span className="text-[10px] uppercase font-black tracking-widest text-zinc-300 group-hover:text-purple-400">{sym}</span>
                      </label>
                    ))}
                  </div>
                )}
              ...
              <button 
                ...
                className={`w-full py-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-3 active:scale-95 border border-white/10 ${
                  environment === 'live' 
                    ...
                    : 'bg-gradient-to-r from-purple-600 to-indigo-500 text-white shadow-[0_0_30px_rgba(147,51,234,0.3)] hover:shadow-[0_0_40px_rgba(147,51,234,0.5)]'
                }`}
              >
