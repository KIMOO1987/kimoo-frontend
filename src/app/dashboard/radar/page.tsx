  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      ...
      for (const s of otherSignals) {
        const { clean } = getSymbolData(s.symbol);
        try {
          const response = await fetch(`https://finnhub.io/api/v1/quote?symbol=${clean}&token=${FINNHUB_KEY}`);
                  ...
                  <tbody className="divide-y divide-white/[0.05]">
                    <AnimatePresence>
                      {filteredRadarSignals.map((signal) => {
                        const cleanSym = signal.symbol.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
                        const current = livePrices[cleanSym] ?? Number(signal.entry_price);
                        const entry = Number(signal.entry_price);
                        const sl = Number(signal.sl);
                        const isBuy = signal.side?.toUpperCase() === 'BUY' || signal.side?.toUpperCase() === 'BULLISH';
                        const rr = Math.abs(entry - sl) ? (isBuy ? (current - entry) : (entry - current)) / Math.abs(entry - sl) : 0;
